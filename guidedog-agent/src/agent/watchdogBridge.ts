...
你在为用户检查一个条件。读取屏幕并判断条件是否满足。
- 条件已满足时，调用 task_complete，简要描述你观察到的内容。
- 条件尚未满足时，调用 task_failed，reason 描述你观察到的“当前状态”
  （不要只写“条件未满足”）——该描述会被记录，并在下一次检查时作为 "previous_check_result" 展示给你，供你对比。
如果上下文中提供了 "previous_check_result"，把当前屏幕状态与它对比。这对“变化型”条件（例如“状态变为 X”）很重要：只有当前状态按描述的方式与上次不同才算满足，而不是仅仅因为当前状态已经符合 X。
除读取屏幕外，不要执行任何操作。
`.trim();
...
function resolveToolPreset(preset: string): string[] {
  switch (preset) {
    case 'read_only':
      return ['ui_inspect', 'ui_screenshot', 'list_apps', 'write_note', 'read_note'];
    case 'navigation':
      return ['ui_tap', 'ui_fill', 'ui_long_press', 'ui_swipe', 'ui_scroll', 'ui_global_action', 'open_app',
        'list_apps', 'ui_find_node', 'wait', 'ui_wait_for_node',
        'ui_wait_for_change', 'ui_get_node', 'ui_inspect', 'ui_screenshot'];
    default:
      return ['ui_inspect', 'ui_screenshot', 'open_app', 'ui_tap', 'ui_fill', 'ui_find_node', 'ui_wait_for_change'];
  }
}
