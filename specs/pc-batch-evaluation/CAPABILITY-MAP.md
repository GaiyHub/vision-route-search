...
| `evidence-collector` | 拉取评测目录内隔离的 OTel/Todo 文件，并采集最终截图、UI 层级和前台包名。 | 现有 OTel/Todo 生成逻辑 | `adb-runner` |
...
- 不新增 evaluation Variant 或独立 APK；普通豆泡 APK 默认提供评测接口，由独立 Android 组件和系统权限限制为 ADB shell 可调用。
