...
**验收标准：**
- [x] 完成、取消和中断 Run 均能产生结构稳定的报告。
- [x] 样本重试后报告按最新 Attempt 重新计算，历史 Attempt 保持可审计。
- [x] 提供计划 Run 报告查询 API，缺失报告返回统一错误。
...
**验收标准：**
- [x] 创建计划时可选择评测集、样本、设备和运行配置，并可从计划启动评测。
- [x] 执行记录按计划与 Run 浏览；样本可查看 Attempt、指标、轨迹并手动重试。
- [x] 每次 Run 可查看整体报告；旧 Run 在历史评测中只读可见。
...
- [x] evaluator 全量 typecheck、tests、build 通过。
- [x] 评测集不再出现执行入口，计划创建到整体报告主流程可用。
- [x] 旧 Run 与移动端用户数据均未发生破坏性迁移。
