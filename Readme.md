...
- 最大步骤支持 1–200，默认 50；提示历史默认保留最近 20 个工具轮次。
- 支持任务超时、模型请求超时、失败重试、停止任务和长任务历史压缩。
...
- 工具可以独立启用或禁用，并可修改界面显示名称、模型可见描述等元信息；规范工具名和参数协议保持稳定。
- 每个工具支持独立的循环预警和阻断阈值。
