...
- 保护最近 4 个完整决策轮次和未配对工具调用。完整 UI 观察不作为持久历史保护对象：模型历史始终保存稳定的短元数据，完整结构树只在紧接着的一次主决策中作为缓存边界之后的瞬态上下文提供。
...
`ui_inspect` 等 UI 结构观察采用稳定历史与瞬态载荷分离：工具协议中的持久结果从首次发送起就是不变的短元数据；完整结构树追加在最新缓存断点之后，不标记缓存，仅供下一次成功的模型决策使用。推理重试复用同一载荷，成功返回后清除。原始 AgentEvent、用户展示和任务日志仍保留完整工具结果。
