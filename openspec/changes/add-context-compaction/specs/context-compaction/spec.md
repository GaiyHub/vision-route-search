...
### Requirement: UI 结构观察仅供下一次决策
系统 MUST 将完整 UI 结构观察与持久工具历史分离；完整结构仅供紧接着的一次成功主模型决策使用，持久历史从首次发送起只包含稳定的有界元数据。

#### Scenario: 调用 UI 结构观察工具
- **WHEN** `ui_inspect` 或原始结构树工具成功返回完整页面结构
- **THEN** 下一次主模型请求在缓存边界之后收到完整结构，同时工具历史只记录 observation ID、页面身份、节点统计和瞬态说明

#### Scenario: 主模型完成下一次决策
- **WHEN** 携带完整结构的主模型请求成功返回
- **THEN** 系统清除该瞬态结构，后续请求不得再次发送其完整内容

#### Scenario: 主模型请求重试
- **WHEN** 携带完整结构的请求发生可恢复错误并重试
- **THEN** 所有重试复用字节一致的 observation ID 和结构内容，直到该决策成功或任务终止

#### Scenario: 保持缓存前缀稳定
- **WHEN** 瞬态结构在一次决策后被清除
- **THEN** 系统不得改写此前已发送的持久工具结果，且完整结构不得位于显式缓存断点之内
