...
describe('评测 WebUI', () => {
  it('以评测计划作为执行入口，并独立展示执行记录和评测集', async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === '/api/android/devices') return Response.json({ devices: [{ serial: 'device-1', model: 'Pixel 8', androidVersion: '15', state: 'ONLINE', canMirror: true }] });
      if (url === '/api/datasets') return Response.json({ datasets: [{ schemaVersion: 1, id: 'smoke', name: '冒烟评测', defaults: { timeoutMs: 30_000, conversationMode: 'ISOLATED' }, samples: [{ id: 'answer', instruction: '现在几点？', enabled: true, assertions: [], judge: { enabled: true, rubric: '回答正确', threshold: 0.8, evidence: ['finalResponse'] } }] }] });
      ...
      if (url === '/api/plans/plan-1' && init?.method === 'DELETE') return new Response(null, { status: 204 });
      if (url === '/api/plans' && init?.method === 'POST') return Response.json(plan, { status: 201 });
      if (url === '/api/runs') return Response.json({ runs: [] });
      if (url === '/api/judge/config') return Response.json({ configured: false, provider: 'OPENAI_COMPATIBLE', hasApiKey: false });
      ...
      if (url === '/api/runs/run-plan-1/report') return Response.json({ planId: 'plan-1', runId: 'run-plan-1', generatedAt: '2026-09-02T08:02:00.000Z', summary: { total: 1, passed: 1, failed: 0, blocked: 0, infraError: 0, timedOut: 0, cancelled: 0, pending: 0, passRate: 1, durationMs: 500, totalTokens: 12, cachedTokens: 0, totalSteps: 3 } });
    ...
    fireEvent.click(screen.getByRole('button', { name: '查看手机实时画面' }));
    expect(await screen.findByRole('dialog', { name: '手机实时画面' })).toBeTruthy();
    expect((await screen.findByAltText('Pixel 8实时画面')).getAttribute('src')).toContain('/api/android/devices/device-1/frame');
    fireEvent.click(screen.getByRole('button', { name: '关闭真机画面' }));
    await waitFor(() => expect(screen.queryByRole('dialog', { name: '手机实时画面' })).toBeNull());
    ...
    const startRequest = fetchMock.mock.calls.find(([input]) => String(input) === '/api/plans/plan-1/runs');
    expect(startRequest?.[1]?.body).toBeUndefined();
    expect(new Headers(startRequest?.[1]?.headers).has('content-type')).toBe(false);
    expect(await screen.findByText('评测计划执行记录')).toBeTruthy();
    expect(await screen.findByText('整体报告')).toBeTruthy();
    expect(screen.getByText('100.0%')).toBeTruthy();
    expect(screen.getByText('总步数')).toBeTruthy();
    expect(screen.getByText('3')).toBeTruthy();
    expect(screen.getByText('Attempt 1')).toBeTruthy();

    fireEvent.click(screen.getByRole('button', { name: '评测集' }));
    expect(await screen.findByText('维护样本、断言与 AI 评审标准，不在此发起执行')).toBeTruthy();
    expect(screen.queryByRole('button', { name: '开始评测' })).toBeNull();
    fireEvent.click(screen.getByRole('button', { name: '配置' }));
    expect(await screen.findByText('LLM-as-Judge')).toBeTruthy();
    fireEvent.click(screen.getByRole('button', { name: '评测计划' }));
    fireEvent.click(await screen.findByRole('button', { name: '删除计划' }));
    expect(globalThis.confirm).toHaveBeenCalledWith('确定删除评测计划“真机核心回归”吗？历史执行记录和报告将保留。');
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith('/api/plans/plan-1', expect.objectContaining({ method: 'DELETE' })));
  });
});
