...
export function PlanConsole({
  ...
  const remove = async () => {
    if (!selected || !globalThis.confirm(`确定删除评测计划“${selected.name}”吗？历史执行记录和报告将保留。`)) return;
    setDeleting(true);
    setError('');
    try {
      await api<void>(`/api/plans/${selected.planId}`, { method: 'DELETE' });
  ...
  return <section className="workspace-grid">
    ...
    <article className="panel plan-detail"><div className="section-head"><div><h2>{selected?.name ?? '选择一个计划'}</h2><p>{selected?.description ?? '计划是发起评测的唯一入口'}</p></div>{selected && <div className="plan-actions"><button onClick={openEdit}>编辑配置</button><button className="danger" disabled={deleting} onClick={() => void remove()}>{deleting ? '删除中…' : '删除计划'}</button></div>}</div>
      {!selected ? <div className="empty"><span>＋</span><h3>创建首个评测计划</h3><p>绑定评测集、设备和执行策略后即可运行</p></div> : <>
        ...
        <div className="samples"><b>执行样本 <i>{selected.sampleIds.length}</i></b>{selected.sampleIds.map((sampleId) => { const sample = dataset?.samples.find((item) => item.id === sampleId); return <div className="sample readonly" key={sampleId}><span><strong>{sample?.name ?? sampleId}</strong><small>{sample?.instruction ?? '样本当前不存在'}</small></span>{sample?.judge?.enabled && selected.judge.enabled && <em>AI 评审</em>}</div>; })}</div>
        {error && <div className="error">{error}</div>}<button className="primary" disabled={starting || device?.state !== 'READY'} onClick={() => void start()}>{starting ? '正在启动…' : device?.state !== 'READY' ? '设备未就绪' : '执行评测计划'}</button>
      </>}
    </article>
