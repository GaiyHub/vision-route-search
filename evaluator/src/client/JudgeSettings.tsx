...
export function JudgeSettings({ api, onConfigured }: { api: ApiClient; onConfigured(configured: boolean): void }) {
  ...
  return <section className="panel judge-settings"><div className="section-head"><div><h2>LLM-as-Judge</h2><p>独立配置 OpenAI-compatible 裁判模型，密钥仅保存在后端进程内存中</p></div><em className={`state ${config?.configured ? 'PASSED' : 'OFFLINE'}`}>{config?.configured ? '已配置' : '未配置'}</em></div>
