...
export function RunConsole({ api, runs, selectedRun, onSelect, onRunUpdated }: {
  ...
  return <section className="workspace-grid runs-workspace"><article className="panel plan-list-panel"><div className="section-head"><div><h2>执行批次</h2><p>当前计划的历史运行</p></div></div><div className="plan-list">{runs.length === 0 ? <div className="empty compact-empty"><p>当前计划暂无执行记录</p></div> : runs.map((run) => <button key={run.runId} className={`plan-card ${run.runId === selectedRun?.runId ? 'active' : ''}`} onClick={() => onSelect(run.runId)}><strong>{run.createdAt ? new Date(run.createdAt).toLocaleString() : '运行批次'}</strong><small>{run.samples.length} 个样本 · {labels[run.state] ?? run.state}</small><em>{run.runId}</em></button>)}</div></article>
...
function ReportSummary({ report }: { report: PlanRunReport }) {
  ...
  return <section className="report-summary"><div className="section-head"><div><h3>整体报告</h3><p>基于每个样本的最新 Attempt</p></div><strong>{(summary.passRate * 100).toFixed(1)}%</strong></div><div className="report-metrics"><span><small>通过</small><b>{summary.passed}/{summary.total}</b></span><span><small>失败/异常</small><b>{summary.failed + summary.infraError + summary.timedOut}</b></span><span><small>总步数</small><b>{summary.totalSteps ?? '不可用'}</b></span><span><small>总 Token</small><b>{summary.totalTokens ?? '不可用'}</b></span><span><small>总耗时</small><b>{formatDuration(summary.durationMs)}</b></span></div></section>;
}
